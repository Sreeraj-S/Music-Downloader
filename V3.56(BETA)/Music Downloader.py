...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.selFormat.set( "mp3" )
        self.L3 = tk.Label(self, text="Format" )
        self.L3.grid(row= 2, column=0)
        self.drop = tk.OptionMenu( self , self.selFormat , *options )
    ...
    def cmdDown(self,song , songFormat):
        if 'https://www.youtube.com' in song:
            try:
                os.system(f'start cmd /c mkdir "youtube cache"')
            except:
                pass
            os.system(f'start cmd /c youtube-dl -o"{self.folderDir}/youtube cache/%(id)s.%(ext)s" -f bestaudio {song}')
            pass
        else:
            os.system(f'start cmd /c spotdl --output-format {songFormat} {song}')
    ...
    def Download(self):
        ...
        self.songFormat = self.selFormat.get()
        self.PRO1['value']=0
        try:
            self.L2.grid_remove()
        except:
        ...
        self.L2.grid_remove()
        self.PRO1.stop()
        self.cmdDown(self.songName, self.songFormat)
        time.sleep(5)
        self.PRO1['value']=100
        self.msga('Completed')
